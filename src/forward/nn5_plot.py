...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 30 14:52:43 2024
...
def train_model(rank, train_loader, val_loader, epochs, device, return_dict, save_path):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.title(f'Training and Validation Loss for Rank {rank}')
    plt.savefig(os.path.join(save_path, f'loss_plot_rank_{rank}.png'))
    plt.close()

    # 保存模型
    torch.save(state_dict_to_cpu(model.state_dict()), os.path.join(save_path, f"model5p_{rank+6}.pth"))
